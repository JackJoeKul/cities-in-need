...
def company_url(tree):
    ...
    # Case 1 - none found
    if tree.xpath('string(.)').find("driver") > -1:
...
def recommends_block(review):
    blocks = review.xpath('.//div[contains(@class, "flex-grid recommends")]//div[contains(@class, "cell")]/span')
    return parse_xpath([''.join(block.xpath('text()')) for block in blocks], unicode, False)

def review_outlook(review):
    return parse_xpath(review.xpath('.//div[contains(@class,"tightLt col span-1-3")][2]//div[contains(@class,"middle")]//div[contains(@class,"cell")]//span[contains(@class,"middle")]//text()'), unicode, False)

def ceo_sentiment(review):
    return parse_xpath(review.xpath(
        './/div[contains(@class,"tightLt col span-1-3")]//div[contains(@class,"middle")]//div[contains(@class,"cell")]//span[contains(@class,"middle")]//span[contains(@class, "showDesk")]//text()'),
                       unicode, False)

def recommends(review):
    return parse_xpath(review.xpath(
        './/div[contains(@class,"tightLt col span-1-3")][1]//div[contains(@class,"middle")]//div[contains(@class,"cell")]//span[contains(@class,"middle")]//text()'),
                       unicode, False)
...
def parse_review(review):
    status_role = employment_status_role(review)[0].split('-')
    return {
        'rating_stars': review_rating_stars(review), 'date': review_date(review),
        'employment': employment(review), 'blocks': review_blocks(review),
         'employment_status':status_role[0],
        'role': status_role[1], 'location':location(review),
        'recomm_outlook': recommends_block(review),
        #'outlook': review_outlook(review), 'recommends': recommends(review),
        'ceo': ceo_sentiment(review)
    }
...
def get_all_reviews(tree, company, reviews_db, max_pages = 257):
